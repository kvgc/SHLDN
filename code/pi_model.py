...
from dataloader import pi_model_train_dataloader, pi_model_unlabeled_dataloader, dataloader, fetch_filenames, fetch_unlabeled_filenames, train_val_split, batch_augment
...
def pseudo_label(images, model):
    boolean = model(images) > 0.5
    labels = tf.cast(boolean, dtype=tf.int32)
...
if __name__ == '__main__':
    ...
    ##########################################################
    train_dataloader = pi_model_train_dataloader(
    ...
    unlabeled_dataloader = pi_model_unlabeled_dataloader(
    ...
    for epoch in range(args.epochs):
        @tf.function
        def train_step(model, batchX, batchU1, batchU2, lambda_u):
            with tf.GradientTape() as tape:
                logitsX = model(batchX[0])
                batchX_one_hot = tf.squeeze(tf.one_hot(batchX[2], 2, dtype=tf.int32))
                lossX = loss_object(batchX_one_hot, logitsX)
                lossU = tf.reduce_mean((model(batchU1)-model(batchU2)) ** 2)
            ...
            train_loss(loss)
            train_accuracy(batchX[2], predictionsX)
        ...
        for batch_num in range(args.val_iterations):
            ...
            train_step(model, batchX, batchU1, batchU2, lambda_u)
    ...
    def test_step(model, images, labels):
        logits = model(images)
        labels_one_hot = tf.expand_dims(tf.squeeze(tf.one_hot(labels, 2, dtype=tf.int32)), axis=0)
