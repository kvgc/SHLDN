...
if __name__ == '__main__':
    ...
    parser.add_argument('--epsilon_adv', type=float, default=0.01)
    ...
    logging.info('epsilon: {}'.format(args.epsilon))
    logging.info('epsilon_adv: {}'.format(args.epsilon_adv))
    ...
    for epoch in range(args.epochs):
        @tf.function()
        def train_step(model, r, epsilon, batchX, batchU, lambda_u):
            with tf.GradientTape() as tape:
                logitsU = model(batchU)
                predictionsU = tf.nn.softmax(logitsU, axis=1)
                logitsUr = model(batchU + r)
                predictionsUr = tf.nn.log_softmax(logitsUr, axis=1)
                loss_adv = tf.reduce_sum(predictionsU * predictionsUr, axis=1)
                loss_adv = -tf.reduce_mean(loss_adv)
            gradient = tape.gradient(loss_adv, r)
            norm = tf.reduce_sum(
                gradient**2, axis=[1, 2, 3], keepdims=True) ** 0.5
            r_adv = tf.math.divide_no_nan(gradient, norm)

            with tf.GradientTape() as tape:
                logitsU = model(batchU)
                predictionsU = tf.nn.softmax(logitsU, axis=1)
                logitsUr = model(batchU + epsilon * tf.stop_gradient(r_adv))
                predictionsUr = tf.nn.log_softmax(logitsUr, axis=1)
                loss_u = tf.reduce_sum(predictionsU * predictionsUr, axis=1)
                ...
                loss_x = loss_object(
                    tf.cast(labelsX_one_hot, tf.float32), logitsX)
        ...
        epsilon_adv = tf.constant(args.epsilon_adv)
        LAMBDA_U = tf.constant(args.lambda_u)
        rampup_length = tf.constant(args.rampup_length)
        ...
        for batch_num in range(args.val_iterations):
            ...
            batchU = tf.concat([batchX[0], batchU], axis=0)
            lambda_u = LAMBDA_U * linear_rampup(epoch, args.rampup_length)
            lambda_u = tf.constant(lambda_u)
            shape = batchU.shape
            if batch_num == 0:
                r = tf.random.normal(shape=shape)
                norm = tf.reduce_sum(
                    r**2, axis=[1, 2, 3], keepdims=True) ** 0.5
                r = epsilon_adv * tf.math.divide_no_nan(r, norm)
                r = tf.Variable(r)
            else:
                d = tf.random.normal(shape=shape)
                norm = tf.reduce_sum(
                    r**2, axis=[1, 2, 3], keepdims=True) ** 0.5
                d = epsilon_adv*tf.math.divide_no_nan(d, norm)
                r.assign(d)

            train_step(model, r, epsilon, batchX, batchU, lambda_u)
